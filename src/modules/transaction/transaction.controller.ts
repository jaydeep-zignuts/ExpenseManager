...
export class TransactionController {
  ...
  async renderAddTransaction(@Param() id: number) {
    return id;
  }
  ...
  async showTransactions(@Param('id') id: number) {
    const acda = await this.transactionService.getAccountById(id);
  ...
  async refShowTransactions(@Param('id') id: number) {
    const acda = await this.transactionService.getAccountById(id);
    const transactions = await this.transactionService.getTransactions(
      acda.acid,
    );
  ...
  async renderEditTransaction(@Param('id') id: number, @Req() req: Request) {
    const transactionData = await this.transactionService.renderEditTransaction(
      id,
    );
  ...
  async editTransaction(
    ...
    const editTransaction = await this.transactionService.editTransaction(
      tid,
  ...
  async getListTransaction(
    ...
    return await this.transactionService.getListTransaction(
      req,
      res,
      acda.acid,
    );
